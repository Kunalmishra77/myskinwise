...
- area: for each feature give a bounding box {x, y, width, height} as fractions 0..1 of the WHOLE image (x,y = top-left corner) marking WHERE on the face you actually see that characteristic. Put it over the real spot — e.g. shine over the forehead/nose, dark circles under the eyes, a blemish on the exact cheek it is on. Keep boxes reasonably small and specific (roughly 0.1–0.35 wide), and give DIFFERENT features their own distinct boxes rather than one big box over the whole face. If two features are in different places, their boxes must differ. Approximate is fine.
