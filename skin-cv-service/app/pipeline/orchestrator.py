...
def analyze_bytes(data: bytes) -> dict[str, Any]:
    ...
    STUB: a pure function of the bytes, so it is deterministic by construction.
    Real measurement replaces the placeholder scores in later phases; the shape
    is the stable contract.
    """
    digest = _sha256(data)

    return {
        # Deterministic id derived from the content, not a random uuid, so the
        # whole body is a pure function of the input.
        ...
        "quality": {
            "passed": True,
            "stub": True,
            "note": "quality gate lands in Phase 2",
        },
        "skin_tone": {
            "median_ita_deg": None,
